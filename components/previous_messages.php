<?php
#we make a call to this page everytime a user scrolls up. 
...
$messages_query =  $con->query("select id,message_from,message,read_yet,date_of,message_type from messages where chat_id = ".$chat_id." order by id desc");
...
$messages_all = $messages_query->fetchAll();
	
if($currently_shown + 15 <= count($messages_all)) {
$messages_all = array_reverse(array_slice($messages_all,$currently_shown, 15));
}
else {
$messages_all = array_reverse(array_slice($messages_all,$currently_shown , 100));	
}
