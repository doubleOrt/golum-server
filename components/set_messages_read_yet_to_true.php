...
$user_can_modify_chat_prepared = $con->prepare("select case when (select chat_id from messages where id = :message_id) in (select id from chats where chatter_ids like concat(:base_user_id, '-%') or chatter_ids like concat('%-', :base_user_id)) then true else false end as user_involved_in_chat");	
...
$con->prepare("update messages set read_yet = 1 where id = :message_id and message_from != :base_user_id")->execute([":message_id" => $_POST["message_id"], ":base_user_id" => $GLOBALS["base_user_id"]]);	
}
}
