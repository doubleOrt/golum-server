...
$prepared = $con->prepare("update users set gender = :gender where id = ".$GLOBALS["base_user_id"]);
...
$prepared = $con->prepare("update users set country = :country where id = ".$GLOBALS["base_user_id"]);
...
$prepared = $con->prepare("update users set birthdate = :birthdate where id = ".$GLOBALS["base_user_id"]);
...
custom_pdo("update avatars set rotate_degree = :avatar_rotation, positions = '0,0' where id_of_user = :base_user_id order by id desc limit 1", [":avatar_rotation" => $_POST["avatar_rotation"], ":base_user_id" => $GLOBALS["base_user_id"]]);		
}	
}
...
custom_pdo("update avatars set positions = :new_positions where id_of_user = :base_user_id order by id desc limit 1", [":new_positions" => ($_POST["avatar_positions"][0] . "," . $_POST["avatar_positions"][1]), ":base_user_id" => $GLOBALS["base_user_id"]]);
}
