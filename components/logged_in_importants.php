...
$user_info_arr =  custom_pdo("select * from users where id = :base_user_id", [":base_user_id" => $_SESSION["user_id"]])->fetch();	
...
$base_user_avatar_arr = custom_pdo("SELECT * FROM avatars WHERE id_of_user = :base_user_id order by id desc limit 1", [":base_user_id" => $_SESSION["user_id"]])->fetch();
