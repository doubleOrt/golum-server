...
if(isset($_GET["comment_id"]) && isset($_GET["last_reply_id"])) {
...
if($_GET["last_reply_id"] == 0) {
$post_comments_arr = $con->query("select * from (SELECT *, @rn:=@rn+1 AS new_id FROM ( SELECT * FROM comment_replies ORDER BY id DESC ) t1, (SELECT @rn:=0) t2) new_table left join (select user_id as user_id2,post_id as post_id2,option_index from post_votes) post_votes on new_table.user_id = post_votes.user_id2 and (select post_id from post_comments where id = new_table.comment_id) = post_votes.post_id2 where comment_id = ". $_GET["comment_id"] . $not_pin_to_top_comment ." limit 15")->fetchAll();	
}
// when the user is infinite scrolling
else {
$post_comments_arr = $con->query("select * from (SELECT *, @rn:=@rn+1 AS new_id FROM ( SELECT * FROM comment_replies ORDER BY id DESC ) t1, (SELECT @rn:=0) t2) new_table left join (select user_id as user_id2,post_id as post_id2,option_index from post_votes) post_votes on new_table.user_id = post_votes.user_id2 and (select post_id from post_comments where id = new_table.comment_id) = post_votes.post_id2  where comment_id = ". $_GET["comment_id"] . $not_pin_to_top_comment ." and new_id > ". $_GET["last_reply_id"] ." limit 15")->fetchAll();	
}


// now we want to append the comment the user wants to pin to the top to the beginning of the $post_comments_arr
if(isset($_GET["pin_comment_to_top"]) && is_numeric($_GET["pin_comment_to_top"])) {
array_unshift($post_comments_arr,$con->query("select * from (SELECT *, @rn:=@rn+1 AS new_id FROM ( SELECT * FROM comment_replies ORDER BY upvotes DESC, id desc ) t1, (SELECT @rn:=0) t2) new_table left join (select user_id as user_id2,post_id as post_id2,option_index from post_votes) post_votes on new_table.user_id = post_votes.user_id2 and (select post_id from post_comments where id = new_table.comment_id) = post_votes.post_id2  where comment_id = ". $_GET["comment_id"] ." and id = ". $_GET["pin_comment_to_top"])->fetch());
}
		

// if there are no comments say there are no comments
if(count($post_comments_arr) < 1 && $_GET["last_reply_id"] == 0) {
$echo_arr[0] = "<div id='noRepliesYet' class='emptyNowPlaceholder'>You Are Going To Be The First To Reply</div>";
}		
else {
