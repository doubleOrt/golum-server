...
$what_id_query = custom_pdo("SELECT id FROM AVATARS where id_of_user = :base_user_id", [":base_user_id" => $GLOBALS["base_user_id"]])->fetchAll();
$what_id = count($what_id_query) > 0 ? $what_id_query[count($what_id_query)-1]["id"] + 1 : 1;


$storagePath = "../users/". $GLOBALS["base_user_id"] ."/media/"; // this is relative to this script, better use absolute path.
$new_name = $what_id;
...
$new_path = "users/". $GLOBALS["base_user_id"] ."/media/" . $new_name . "." . $upload_result[1];	

//add a new row to the avatars table, check if it is successful.
$insert_into_avatars = custom_pdo("INSERT INTO AVATARS (id_of_user,avatar_path,date_of) values(:base_user_id, :new_path, :date_of)", [":base_user_id" => $GLOBALS["base_user_id"], ":new_path" => $new_path, ":date_of" => date("Y/m/d H:i")]);

//if query was successful
if($insert_into_avatars->rowCount() > 0) {
	
//change the user's avatar_picture column in the users table
$update_users = custom_pdo("UPDATE USERS SET avatar_picture = :new_path where id = :base_user_id", [":new_path" => $new_path, ":base_user_id" => $GLOBALS["base_user_id"]]);
...
global $con;	
$background_uploads_today = custom_pdo("SELECT count(*) from avatars where id_of_user = :base_user_id and date_of like concat(:date_of,' %')", [":base_user_id" => $GLOBALS["base_user_id"], ":date_of" => date("Y/m/d")])->fetch()[0];
