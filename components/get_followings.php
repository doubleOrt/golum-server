...
$all_contacts_arr = $con->query("select users.id, first_name, last_name, user_name, gender, avatar_picture, contact from contacts left join users on contacts.contact = users.id where contact_of = ". $_SESSION["user_id"] ." order by first_name asc")->fetchAll();
...
<br>
After You Add Some Follows, They Will Appear Here
</div>";	
}
else {
